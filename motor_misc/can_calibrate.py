...
async def main():
    parser = argparse.ArgumentParser(
        description="Script to calibrate the ODrive over CAN bus."
    )
    parser.add_argument(
        "-i",
        "--interface",
    ...
    with can.interface.Bus(
        ...
        with CanSimpleNode(bus=bus, node_id=args.node_id) as node:
            ...
            while True:
                ...
                error, state, procedure_result, traj_done = struct.unpack(
            ...
            if error != 0 or procedure_result != 0:
                raise Exception(
                    f"Calibration resulted in {error=}, {procedure_result=}. See here for error codes:\n"
