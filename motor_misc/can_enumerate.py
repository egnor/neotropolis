...
class Discoverer:
    ...
    def assign_free_node_id(self, serial_number):
        next_free_node_id = next(
            i
            for i in range(len(self.discovered_devices) + 1)
            if i not in self.discovered_devices.values()
        )
        if next_free_node_id > MAX_NODE_ID:
            print(
                f"Can't address {sn_str(serial_number)} because there are too many devices on the bus."
            )

        print(
            f"Assigning node ID {next_free_node_id} to {sn_str(serial_number)}"
        )
    ...
    def on_message_received(self, msg: can.Message):
        ...
        if cmd_id == ADDRESS_CMD and not msg.is_remote_frame:
            ...
            if serial_number not in self.discovered_devices:
                ...
                print(
                    f"Discovered ODrive {sn_str(serial_number)} ({node_id_str})"
                )
            # else:
            ...
            self.discovered_devices[serial_number] = (
                node_id if node_id != BROADCAST_NODE_ID else None
            )
...
async def scan_for_devices(bus):
    ...
    print(
        f"Scan complete. Discovered {len(discoverer.discovered_devices)} ODrives.\n"
    )
    return discoverer.discovered_devices


def identify_ui(
    bus: can.Bus, node_ids: List[int], user_labels: List[str]
) -> Dict[str, int]:
    """
    ...
    unidentified_node_ids = [
        node_id for node_id in node_ids if not node_id in node_to_label
    ]
    ...
    if len(unidentified_node_ids):
        print(
            f"Some unidentified ODrives were found: node_ids {unidentified_node_ids}"
        )
    if len(user_labels):
...
async def main():
    parser = argparse.ArgumentParser(
        description="Interactive ODrive identification script."
    )
    parser.add_argument(
        "-i",
        "--interface",
    ...
    parser.add_argument(
        ...
        type=int,
        default=250000,
        help="Bitrate for CAN bus. Default is 250000.",
    )
    ...
    with can.interface.Bus(
        ...
        if len(user_labels):
            ok, node_to_label = identify_ui(
                bus, discovered_devices.values(), user_labels
            )
            ...
            sn_to_new_addr = [
                ...
                if node_id != discovered_devices[sn]
            ]

            if len(sn_to_new_addr):
                print(
                    f"Assigning new node IDs to {len(sn_to_new_addr)} ODrives"
                )
                await set_addresses(bus, sn_to_new_addr)

            if len(sn_to_target_addr) and args.save_config:
                print(
                    f"Saving configuration on {len(sn_to_target_addr)} ODrives"
                )
                for sn, node_id in sn_to_target_addr:
        ...
        await asyncio.sleep(
            0.1
        )  # needed for last message to get through on SLCAN backend
