...
import math
import struct
from can_simple_utils import (
    CanSimpleNode,
    REBOOT_ACTION_SAVE,
)  # if this import fails, make sure you copy the whole folder from the git repository
...
class EndpointAccess:
    ...
    async def version_check(self):
        ...
        if self.endpoint_data["fw_version"] != fw_version_str:
            raise Exception(
                f"the file provided in --endpoints-json does not match the firmware version of the ODrive: {self.endpoint_data['fw_version']} != {fw_version_str}"
            )
        if self.endpoint_data["hw_version"] != hw_version_str:
            raise Exception(
                f"the file provided in --endpoints-json does not match the firmware version of the ODrive: {self.endpoint_data['hw_version']} != {hw_version_str}"
            )
    ...
    async def write_and_verify(self, path: str, val):
        ...
        self.node.bus.send(
            can.Message(
                ...
                data=struct.pack(
                    "<BHB" + endpoint_fmt, _OPCODE_WRITE, endpoint_id, 0, val
                ),
                is_extended_id=False,
            )
        )
        ...
        _, _, _, return_value = struct.unpack_from(
            "<BHB" + endpoint_fmt, msg.data
        )
        val_pruned = (
            val
            if endpoint_type != "float"
            else struct.unpack("<f", struct.pack("<f", val))[0]
        )
        matches = (
            math.isnan(return_value)
            if math.isnan(val_pruned)
            else return_value == val_pruned
        )
        if not matches:
            raise Exception(
                f"failed to write {path}: {return_value} != {val_pruned}"
            )
...
async def main():
    parser = argparse.ArgumentParser(
        description="Script to configure ODrive over CAN bus."
    )
    parser.add_argument(
        "-i",
        "--interface",
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to flat_endpoints.json corresponding to the given ODrive and firmware version.",
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        "--save-config",
        action="store_true",
        help="Save the configuration to NVM and reboot ODrive.",
    )
